...
export class FoodCatalogueComponent {
  ...
  restaurantId: number;
  foodItemResponse: FoodCataloguePage; //FoodCataloguePage (Restaurant +FoodItem[5]) - get from db

  foodItemCart: FoodItem[] = []; //add to care list to purchess
  orderSummary: FoodCataloguePage; //FoodCataloguePage (Restaurant +FoodItem[2]) -- sellected food upload to order-summary

 //route: ActivatedRoute ===> (where the call came from) [can get the parameter value from that]
 //router: Router  =====> where we have to router to next page, with parameter
  ...
  increment(food: any) {
    food.quantity++;
    const index = this.foodItemCart.findIndex(item => item.id === food.id); // find correct food_item object
    //check in foodItemCart avaliablelity of food.id
    if (index === -1) {
